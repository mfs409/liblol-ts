...
module LOL {
    /**
     * Enum for tracking the type of a shape.  LOL supports three shape types, 
     * box, circle, and polygon
     */
    enum SHAPETYPE { BOX = 1, CIRCLE, POLYGON };
    ...
    export abstract class Actor extends Renderable {
         ...
         */
        public body: PhysicsType2d.Dynamics.Body = null;

        /**
         * The dimensions of the Actor in meters... x is width, y is height
         */
        public size: PhysicsType2d.Vector2 = new PhysicsType2d.Vector2(0, 0);

        /**
          * Track the underlying shape (circle, box, polygon)
          */
        private shapeType = SHAPETYPE.BOX;
         ...
         */
        constructor(imgName: string, width: number, height: number, game: Lol) {
            // Create a renderable for this Actor.  Note that the x/y coordinates 
            // are temporary... when we start actually rendering, we'll get this 
            // right.
            super(0, 0, width*game.config.PIXELS_PER_METER, height*game.config.PIXELS_PER_METER, imgName, game);
         ...
         */
        public abstract onCollide(other: Actor, contact: PhysicsType2d.Dynamics.Contacts.Contact);
         ...
         * @param game         The game in which this actor's body is being changed
         */
        public setBoxPhysics(x: number, y: number, type: PhysicsType2d.Dynamics.BodyType, game: Lol) {
            ...
            boxBodyDef.position.Set(x + this.size.x / 2, y + this.size.y / 2);
            this.body = game.activeLevel.world.CreateBody(boxBodyDef);
